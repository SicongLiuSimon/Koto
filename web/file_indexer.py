...
class FileIndexer:
    ...
    def index_file(
        self, file_path: str, tags: List[str] = None, metadata: Dict = None
    ) -> Dict[str, Any]:
        """
        索引单个文件
    ...
    def index_directory(
        self, directory: str, recursive: bool = True, extensions: List[str] = None
    ) -> Dict[str, Any]:
        """
        批量索引目录
    ...
    def search(
        self, query: str, limit: int = 20, file_types: List[str] = None
    ) -> List[Dict[str, Any]]:
        """
        全文搜索
    ...
    def _generate_snippet(
        ...
        # 高亮关键词
        snippet = re.sub(
            f"({re.escape(query)})", r"**\1**", snippet, flags=re.IGNORECASE
        )
    ...
    def find_by_content(
        self, content_sample: str, min_similarity: float = 0.5
    ) -> List[Dict[str, Any]]:
        """
        根据内容片段查找相似文件
    ...
    def remove_file(self, file_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            cursor.execute(
                "DELETE FROM file_content_fts WHERE file_path = ?", (file_path,)
            )
    ...
    def rebuild_index(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": True, "message": "索引重建完成", "stats": result}
...
def test_file_indexer():
    ...
    logger.info(
        f"   结果: 总数={result.get('total')}, 已索引={result.get('indexed')}, 跳过={result.get('skipped')}\n"
    )
    ...
    for i, r in enumerate(results[:3], 1):
        logger.info(
            f"   {i}. {r['file_name']} (相似度: {r.get('similarity', 0):.2f})\n"
        )
