...
class ProactiveTriggerSystem:
    ...
    def _register_builtin_triggers(self):
        ...
        self.trigger_params["event_return_after_break"] = {"break_timeout_hours": 4}
    ...
    def evaluate_interaction_need(
        ...
        for trigger_id, trigger in self.triggers.items():
            ...
            try:
                ...
                # 计算最终分数
                final_score = (urgency * 0.4 + importance * 0.4) - (
                    disturbance_cost * 0.2
                )
        ...
        # 如果接受率高于70%，可以缩短冷却时间
            ...
            if trigger_id in self.triggers:
                self.triggers[trigger_id].cooldown_minutes = max(
                    15, int(self.triggers[trigger_id].cooldown_minutes * 0.8)
                )
    ...
    def _check_pending_suggestions(
        self, user_id: str
    ...
    def _check_work_duration(self, user_id: str) -> Optional[Tuple[float, float, Dict]]:
        ...
        today_events = [
            e for e in events if e["timestamp"].startswith(str(datetime.now().date()))
        ]
        ...
        if len(today_events) > 0:
            ...
            if work_hours >= work_duration_threshold:
                urgency = min(
                    0.5 + (work_hours - work_duration_threshold) * urgency_per_hour,
                    max_urgency,
                )
                importance = 0.8
    ...
    def _check_edit_frequency(
        ...
        if edit_events:
            ...
            file_edits = Counter(
                e["file_path"] for e in edit_events if e.get("file_path")
            )
    ...
    def _check_goal_updates(self, user_id: str) -> Optional[Tuple[float, float, Dict]]:
        ...
        try:
            ...
            if check_waiting:
                ...
                for g in waiting_goals:
                    try:
                        ...
                        ).total_seconds() > waiting_timeout * 60:
                            ctx = g.get_context()
            ...
            urgency = (
                0.75 if any("等待" in m or "需要" in m for m in messages) else 0.55
            )
            content = {
                "title": "你委托的事情有新进展",
    ...
    def start_monitoring(self, check_interval: int = 300, user_id: str = "default"):
        ...
        self.check_thread = threading.Thread(
            target=self._monitoring_loop, args=(check_interval, user_id), daemon=True
        )
        ...
        while self.running:
            try:
                ...
                if decision and decision.should_interact:
                    ...
                    logger.info(
                        f"  (紧急:{decision.urgency_score:.2f} + 重要:{decision.importance_score:.2f} - 打扰:{decision.disturbance_cost:.2f})"
                    )
