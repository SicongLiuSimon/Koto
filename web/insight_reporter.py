...
class InsightReporter:
    ...
    def _ensure_db(self):
        ...
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_trend_time ON trend_data(recorded_at DESC)"
        )
    ...
    def generate_weekly_report(self) -> Dict:
        ...
        return self._generate_report(self.REPORT_WEEKLY, start_date, end_date)
    ...
    def generate_monthly_report(self) -> Dict:
        ...
        return self._generate_report(self.REPORT_MONTHLY, start_date, end_date)
    ...
    def _generate_report(
        ...
        # 1. 活动概览
        report["sections"]["activity_overview"] = self._get_activity_overview(
            start_date, end_date
        )

        # 2. 文件操作统计
        report["sections"]["file_operations"] = self._get_file_operations_stats(
            start_date, end_date
        )

        # 3. 生产力分析
        report["sections"]["productivity"] = self._get_productivity_analysis(
            start_date, end_date
        )
    ...
    def _get_activity_overview(self, start_date: datetime, end_date: datetime) -> Dict:
        """获取活动概览"""
        # 获取每日活动
        daily_activity = self.behavior_monitor.get_daily_activity(
            days=(end_date - start_date).days
        )
        ...
        # 最活跃的一天
        most_active_day = (
            max(daily_activity, key=lambda x: x["event_count"])
            if daily_activity
            else None
        )
    ...
    def _get_file_operations_stats(
        ...
        return {
            ...
            "most_common_operation": (
                operation_counts.most_common(1)[0] if operation_counts else None
            ),
        }

    def _get_productivity_analysis(
        self, start_date: datetime, end_date: datetime
    ) -> Dict:
        """生产力分析"""
        # 获取文件编辑次数
        ...
        return {
            "total_files_edited": len(
                [f for f in frequent_files if f.get("edit_count", 0) > 0]
            ),
    ...
    def _get_knowledge_insights(self) -> Dict:
        ...
        try:
            ...
            # 获取热门概念
            top_concepts = self.knowledge_graph.concept_extractor.get_top_concepts(
                limit=10
            )
            ...
            return {
                ...
                "insight": self._interpret_graph_density(
                    kg_stats.get("graph_density", 0)
                ),
            }
    ...
    def _get_search_analysis(self) -> Dict:
        ...
        searches_with_click = len(
            [s for s in search_history if s.get("clicked_result")]
        )
    ...
    def _get_trends_comparison(self, report_type: str) -> Dict:
        ...
        try:
            ...
            # 对比关键指标
            current_events = current_report["sections"]["activity_overview"][
                "total_events"
            ]
            previous_events = previous_report["sections"]["activity_overview"][
                "total_events"
            ]

            change_percent = (
                (current_events - previous_events) / max(previous_events, 1)
            ) * 100
