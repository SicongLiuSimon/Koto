...
class ConceptExtractor:
    ...
    def _ensure_db(self):
        ...
        # 创建索引加速查询
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_file_concepts_path ON file_concepts(file_path)"
        )
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_file_concepts_concept ON file_concepts(concept)"
        )
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_file_concepts_score ON file_concepts(tf_idf_score DESC)"
        )
    ...
    def get_idf(self, concept: str) -> float:
        ...
        # 获取包含该概念的文档数
        cursor.execute(
            "SELECT document_frequency FROM concept_stats WHERE concept = ?", (concept,)
        )
    ...
    def analyze_file(self, file_path: str, content: str = None) -> Dict:
        ...
        # 保存到数据库
        self._save_concepts(
            file_path, concepts, content_hash, len(self.tokenize(content))
        )
    ...
    def get_file_concepts(self, file_path: str, limit: int = 10) -> List[Dict]:
        ...
        for row in cursor.fetchall():
            concepts.append(
                {"concept": row[0], "score": row[1], "extracted_at": row[2]}
            )
    ...
    def find_related_files(self, file_path: str, limit: int = 5) -> List[Dict]:
        ...
        for other_file, other_concepts in file_vectors.items():
            # 计算共享概念的加权得分
            shared_score = sum(
                file_concepts.get(c, 0) * score for c, score in other_concepts.items()
            )
