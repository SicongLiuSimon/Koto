    ...
    def sanitize_text(text: str, context: str = "general") -> str:
        ...
        if context in ("ppt_point", "ppt_title", "general"):
            for pattern, replacement in MARKDOWN_RESIDUE_PATTERNS:
                new_text = re.sub(pattern, replacement, text, flags=re.MULTILINE).strip()
                if new_text != text:
                    was_modified = True
                    text = new_text
            ...
            for side in ('left', 'right'):
                ...
                if side_data and isinstance(side_data, dict):
                    for k in ('subtitle', 'label'):
                        if k in side_data:
                            side_data[k] = ContentSanitizer.sanitize_text(side_data[k], "ppt_title")
                    if 'points' in side_data:
                        side_data['points'] = [
                            ContentSanitizer.sanitize_text(p, "ppt_point")
                            for p in side_data['points']
                            if isinstance(p, str) and ContentSanitizer.sanitize_text(p, "ppt_point")
                        ]
...
class FileQualityEvaluator:
    ...
    @classmethod
    def evaluate_ppt_outline(cls, outline: list, user_request: str = "") -> Dict[str, Any]:
        """
        评估 PPT 大纲质量（在渲染前调用）。
        ...
        if user_request:
            all_text = ' '.join(
                (s.get('title', '') + ' ' + ' '.join(s.get('points', []) if isinstance(s.get('points', []), list) else []))
                for s in outline
            )
            # 提取用户请求中的关键词
    ...
    @classmethod
    def evaluate_document_text(cls, text: str, user_request: str = "") -> Dict[str, Any]:
        ...
        if not text or not text.strip():
            return {'score': 0, 'pass': False, 'issues': ['文档内容为空'], 'suggestions': ['重新生成文档']}
    ...
    def evaluate_pptx_file(cls, pptx_path: str) -> Dict[str, Any]:
        ...
        if not os.path.exists(pptx_path):
            return {'score': 0, 'pass': False, 'issues': ['文件不存在'], 'suggestions': []}

        try:
            from pptx import Presentation
        except ImportError:
            return {'score': 50, 'pass': True, 'issues': ['无法检查: python-pptx 未安装'],
                    'suggestions': []}
...
class FileQualityGate:
    ...
    @staticmethod
    def check_and_fix_ppt_outline(outline: list, user_request: str = "",
        ...
        if fixes and progress_callback:
            progress_callback(f"🧹 已清洗 {len(fixes)} 处内容问题", "移除 Markdown 残留和 AI 对话痕迹")
    ...
    @staticmethod
    def check_and_fix_document(text: str, user_request: str = "",
                                progress_callback=None) -> Dict[str, Any]:
        """
        文档内容质量门控：清洗 → 评估 → 返回结果。
        """
    ...
    def post_check_pptx(pptx_path: str, progress_callback=None) -> Dict[str, Any]:
        ...
        if progress_callback:
            ...
            progress_callback(
                ...
                "; ".join(quality['issues'][:3]) if quality['issues'] else "文件质量良好"
            )
    ...
    def check_and_fix_for_export(
        ...
        if not text:
            return {
                'text': text, 'issues': [], 'fixes': [],
                'stripped_md': False,
                'quality': {'score': 0, 'pass': False, 'issues': ['内容为空'], 'suggestions': []}
            }
        ...
        if md_issues and progress_callback:
            progress_callback(
                f"🔍 检测到 {len(md_issues)} 类 Markdown 符号",
                "; ".join(md_issues[:3])
            )
        ...
        if target_format == "excel":
            ...
            if stripped_md and progress_callback:
                progress_callback("🧹 Excel 导出：已去除所有 Markdown 符号", "单元格只保留纯文字")
            ...
            if md_issues and progress_callback:
                ...
                if md_ok:
                    progress_callback("🔍 Word 导出：Markdown 格式将由排版引擎处理", "已清理孤立符号")
                stripped_md = True

        # Step 4: 质量评估
        quality = FileQualityEvaluator.evaluate_document_text(sanitized, user_request)
        if fixes:
            quality['issues'] = [f"已清洗 {len(fixes)} 处 AI 噪声"] + quality.get('issues', [])
