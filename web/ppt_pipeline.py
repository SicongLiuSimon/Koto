...
class PPTGenerationPipeline:
    ...
    async def generate(
        ...
        **kwargs,
        ...
        try:
            ...
            # 1.1 获取初步大纲
            _think(
                f"正在分析文档内容 ({len(user_request)} chars)... 提取核心论点与关键数据"
            )
            ...
            sections = [
                s.title for s in blueprint.slides if s.slide_type.value == "section"
            ]
            if sections:
            ...
            special_slides = [
                ...
                if s.slide_type.value in ["comparison", "data", "flow"]
            ]
            if special_slides:
            ...
            # 阶段2: 质量检查
            _report(
                f"【阶段2】正在检查第 {blueprint.slides[0].slide_index}-{blueprint.slides[-1].slide_index} 页的内容一致性...",
                45,
            )

            # 使用更详细的提示，模拟"思考"过程
            quality_check = await PPTQualityEnsurance.verify_blueprint_quality(
                blueprint
            )
            ...
            if quality_check["recommendations"]:
                rec_str = "\n".join(
                    [f"- {r}" for r in quality_check["recommendations"][:2]]
                )
                thought_msg += f"\n\n正在执行优化策略：\n{rec_str}"
            ...
            _think(
                f"✅ 文件已生成。幻灯片总数：{synthesis_result.get('slide_count')}。"
            )
            ...
            final_result = self._finalize_result(
                synthesis_result, blueprint, quality_check, output_path
            )
    ...
    def _prepare_image_map(
        self, blueprint: PPTBlueprint, existing_images: Optional[List[str]] = None
    ) -> Dict[int, List[str]]:
        """
        为蓝图中的幻灯片映射图像
        ...
        if existing_images:
            ...
            for slide in blueprint.slides:
                ...
                if slide.slide_index not in image_map:
                    if slide.image_prompts or slide.slide_type.value in [
                        "content_image",
                        "image_full",
                    ]:
    ...
    def _acquire_images_for_blueprint(self, blueprint: PPTBlueprint):
        ...
        try:
            ...
            if (
                self.orchestrator
                and hasattr(self.orchestrator, "_report")
                and hasattr(self.orchestrator, "_think")
            ):
            ...
            for slide in blueprint.slides:
                ...
                if slide.image_prompts and not slide.image_paths:
                    ...
                    self._log(
                        f"   🎨 正在为第 {slide.slide_index} 页配图: {prompt[:20]}..."
                    )
    ...
    def _finalize_result(
        ...
        return {
            ...
            "blueprint_info": {
                ...
                "layout_types": list(set(s.slide_type.value for s in blueprint.slides)),
            },
