...
class NotificationManager:
    ...
    def __init__(self, db_path: str = "config/notifications.db"):
        ...
        self.connections: Dict[str, Set] = defaultdict(set)  # user_id -> set of websocket connections
    ...
    def _load_preferences(self):
        ...
        for row in rows:
            ...
            self.user_preferences[user_id] = {
                'enabled_types': set(row[1].split(',')) if row[1] else set(self.NOTIFICATION_TYPES.keys()),
    ...
    def send_notification(
        ...
        """
        # 检查用户偏好
        if not force_send and not self._should_send(user_id, notification_type, priority):
            return None
    ...
    def mark_as_read(self, notification_id: int, user_id: str):
        ...
        if cursor.rowcount > 0:
            # 获取通知类型
            cursor.execute("SELECT type FROM notifications WHERE id = ?", (notification_id,))
            row = cursor.fetchone()
            if row:
    ...
    def update_user_preferences(self, user_id: str, preferences: Dict):
        ...
        enabled_types = ','.join(preferences.get('enabled_types', list(self.NOTIFICATION_TYPES.keys())))
...
def get_notification_manager(db_path: str = "config/notifications.db") -> NotificationManager:
    """获取通知管理器实例（单例）"""
