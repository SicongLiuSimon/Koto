...
class LocalExecutor:
                ...
                if app_path:
                    try:
                        if app_path.startswith("ms-"):
                            ...
                            subprocess.Popen(f'start shell:AppsFolder\\{app_path}', shell=True,
                                             stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
                    ...
                    try:
                        if sys.platform == "win32":
                            ...
                            if not proc_name.endswith(".exe"):
                                proc_name += ".exe"
                            ret = subprocess.run(
                                f'taskkill /IM "{proc_name}" /F', shell=True,
    ...
    def get_clipboard(cls):
        ...
        try:
            ...
            return {"success": False, "content": "", "message": f"❌ 无法读取剪贴板: {str(e)}"}
        ...
        try:
            ...
            for proc in psutil.process_iter(['pid', 'name']):
                try:
                    ...
                    continue
            return {"success": True, "apps": apps[:30], "count": len(apps),
    ...
    def open_file_or_directory(cls, path):
        ...
        try:
            ...
            return {"success": True, "message": f"✅ 已打开{kind}: {os.path.basename(path) or path}"}
        except Exception as e:
