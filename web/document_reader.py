    ...
    def read_ppt(file_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for idx, slide in enumerate(prs.slides):
                slide_info = {"index": idx, "title": "", "content": [], "notes": ""}
            ...
            return {"success": False, "error": f"读取PPT失败: {str(e)}"}
        ...
        try:
            ...
            for child in _body_children:
                ...
                if tag == "p":
                    ...
                    has_image = bool(
                        child.findall(".//" + _qn("w:drawing"))
                        or child.findall(".//" + _qn("w:pict"))
                    )
                    ...
                    if _table_idx < len(doc.tables):
                        ...
                        table_data = {"index": _table_idx, "rows": []}
            ...
            return {"success": False, "error": f"读取Word失败: {str(e)}"}
    ...
    def format_for_ai(doc_data: Dict[str, Any]) -> str:
        ...
        if doc_type == "ppt":
            ...
            for slide in doc_data.get("slides", []):
                output.append(
                    f"### 幻灯片 {slide['index'] + 1}: {slide['title'] or '(无标题)'}"
                )
            ...
            for t in doc_data.get("tables", []):
                ...
                for r in rows[1:]:
                    body_rows.append(
                        "| " + " | ".join(str(c) if c else "" for c in r) + " |"
                    )
            ...
            for sheet in doc_data.get("sheets", []):
                ...
                output.append(
                    f"**尺寸**: {sheet['row_count']}行 × {sheet['col_count']}列\n"
                )
