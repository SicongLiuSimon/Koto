...
class FastVoiceRecognizer:
    ...
    def _start_background_init(self):
        ...
        if (
            self.primary_engine == "vosk"
            or "speech_recognition" in self.available_engines
        ):
    ...
    def recognize(self, timeout: int = 5, language: str = "zh-CN") -> VoiceResult:
        ...
        # 本地录音（无网络Google API）
        if (
            "windows_sapi" in self.available_engines
            or "speech_recognition" in self.available_engines
        ):
    ...
    def _recognize_with_vosk(
        ...
        try:
            ...
            if not self.vosk_model:
                return VoiceResult(
                    success=False, message="Vosk模型未加载", engine="vosk"
                )
            ...
            return VoiceResult(
                success=False, message=f"Vosk识别错误: {str(e)}", engine="vosk"
            )
    ...
    def _recognize_with_sr_google(self, timeout: int, language: str) -> VoiceResult:
        ...
        try:
            ...
            return VoiceResult(
                success=False, message=f"识别失败: {str(e)}", engine="error"
            )
        ...
        try:
            ...
            with sr.Microphone(sample_rate=16000) as source:
                ...
                try:
                    ...
                    return VoiceResult(
                        success=False, message=f"超时未检测到语音", engine="timeout"
                    )
        except Exception as e:
            return VoiceResult(
                success=False, message=f"录音失败: {str(e)}", engine="error"
            )
    ...
    def get_available_engines(self) -> Dict:
        ...
        return {"success": True, "engines": engines, "primary": self.primary_engine}
    ...
    try:
        ...
        with sr.Microphone(sample_rate=16000) as source:
            ...
            if accumulated_audio and final_text:
                yield {
                    "type": "final",
                    "text": _clean_chinese_text(final_text),
                    "engine": "google_streaming",
                }
            elif not is_speaking:
                yield {
                    "type": "error",
