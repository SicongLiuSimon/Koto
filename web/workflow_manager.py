...
class Workflow:
    ...
    def set_variable(
        self, var_name: str, default_value: Any = None, description: str = ""
    ):
        """定义工作流变量"""
    ...
    def from_dict(data: Dict) -> "Workflow":
        """从字典创建"""
        wf = Workflow(
            data.get("name", ""), data.get("description", ""), data.get("steps", [])
        )
...
class WorkflowManager:
    ...
    def list_workflows(
        self, tags: List[str] = None, is_template: bool = None
    ) -> List[Workflow]:
        """列出工作流"""
        results = []
    ...
    def create_workflow(
        self, name: str, description: str = "", steps: List[Dict] = None
    ) -> Workflow:
        """创建新工作流"""
    ...
    def clone_workflow(self, source_id: str, new_name: str) -> Optional[Workflow]:
        ...
        cloned = Workflow(
            new_name, source.description, [step.copy() for step in source.steps]
        )
    ...
    def _get_most_used(self, limit: int = 3) -> List[Dict]:
        """获取最常用的工作流"""
        sorted_wfs = sorted(
            self.workflows.values(), key=lambda x: x.execution_count, reverse=True
        )
        return [
            {"name": w.name, "executions": w.execution_count}
            for w in sorted_wfs[:limit]
        ]

    def _get_recently_updated(self, limit: int = 3) -> List[Dict]:
        """获取最近更新的工作流"""
        sorted_wfs = sorted(
            self.workflows.values(), key=lambda x: x.updated_at, reverse=True
        )
...
class WorkflowExecutor:
    ...
    def execute(
        ...
        try:
            ...
            execution["duration"] = (
                datetime.now() - datetime.fromisoformat(execution["start_time"])
            ).total_seconds()
        ...
        "steps": [
            {
                "name": "收集信息",
                "type": "agent",
                "config": {"request": "总结今天完成的工作"},
            },
            {
                "name": "组织内容",
                "type": "tool",
                "config": {"tool": "organize_text", "args": {}},
            },
            {
                "name": "生成报告",
                "type": "agent",
                "config": {"request": "生成最终报告"},
            },
        ],
        "tags": ["report", "daily"],
        "is_template": True,
        ...
        "steps": [
            {
                "name": "分析需求",
                "type": "agent",
                "config": {"request": "分析项目需求"},
            },
            {
                "name": "制定任务",
                "type": "agent",
                "config": {"request": "制定任务清单"},
            },
            {
                "name": "分配资源",
                "type": "tool",
                "config": {"tool": "allocate_resources"},
            },
            {
                "name": "制定时间表",
                "type": "agent",
                "config": {"request": "制定项目时间表"},
            },
        ],
...
def create_template_workflows(workflow_manager: WorkflowManager):
    ...
    for template_id, template_data in WORKFLOW_TEMPLATES.items():
        wf = Workflow(
            template_data["name"], template_data["description"], template_data["steps"]
        )
