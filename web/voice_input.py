...
class VoiceInputEngine:
    ...
    def _check_windows_speech(self) -> bool:
        ...
        try:
            ...
            try:
                ...
                logger.debug("Windows speech COM object unavailable: %s", e)
                return False
        except ImportError:
            return False
    ...
    def _check_gemini_api(self) -> bool:
        ...
        try:
            ...
            logger.debug("Failed to check Gemini API config: %s", e)
            return False
    ...
    def recognize_microphone(
        self, timeout: int = 5, language: str = "zh-CN"
    ) -> RecognitionResult:
        """从麦克风实时识别 - 优先使用本地引擎"""
        # 优先使用 Vosk 本地识别（最快）
    ...
    def _clean_chinese_text(self, text: str) -> str:
        ...
        while re.search(f"({chinese_pattern})\\s+({chinese_pattern})", result):
            result = re.sub(
                f"({chinese_pattern})\\s+({chinese_pattern})", r"\1\2", result
            )
        return result.strip()

    def _recognize_with_vosk(
        self, timeout: int = 5, language: str = "zh-CN"
    ) -> RecognitionResult:
        ...
        try:
            ...
            try:
                ...
                logger.debug("Vosk advanced features not supported: %s", e)
            ...
            return RecognitionResult(
                success=False, message=f"Vosk 识别错误: {str(e)}", engine="vosk"
            )
    ...
    def recognize_streaming(self, timeout: int = 10, language: str = "zh-CN"):
        ...
        try:
            ...
            if not self.vosk_model:
                ...
                if not model_path:
                    # 降级到非流式
                    result = self.recognize_microphone(
            ...
            try:
                while True:
                    ...
                    if rec.AcceptWaveform(data):
                        ...
                        partial = json.loads(rec.PartialResult())
                        partial_text = self._clean_chinese_text(
                            partial.get("partial", "")
                        )
    ...
    def _recognize_with_windows_sapi(
        self, timeout: int = 5, language: str = "zh-CN"
    ) -> RecognitionResult:
        """使用 Windows SAPI 本地识别 - 完全离线，超快响应"""
        if sys.platform != "win32":
            return RecognitionResult(
                success=False, message="仅支持 Windows", engine="windows"
            )
    ...
    def _recognize_with_google(
        ...
        try:
            ...
            return RecognitionResult(
                success=False, message=f"语音识别库未安装: {str(e)}", engine="google"
            )
        ...
        try:
            ...
            return RecognitionResult(
                success=False, message="未检测到语音，请靠近麦克风说话", engine="google"
            )
        except sr.UnknownValueError:
            return RecognitionResult(
                success=False, message="无法识别，请清晰说话后重试", engine="google"
            )
        except sr.RequestError as e:
            return RecognitionResult(
                success=False, message=f"网络请求失败: {str(e)}", engine="google"
            )
        except Exception as e:
            return RecognitionResult(
                success=False, message=f"识别错误: {str(e)}", engine="google"
            )

    def recognize_audio_file(
        self, audio_path: str, engine: Optional[str] = None
    ) -> RecognitionResult:
        """识别音频文件"""
        try:
            target_engine = (
                self._parse_engine(engine) if engine else self.primary_engine
            )
            ...
            return RecognitionResult(
                success=False, message=f"音频识别错误: {str(e)}", engine="error"
            )
    ...
    def _recognize_with_gemini(self, audio_path: str) -> RecognitionResult:
        ...
        try:
            ...
            if not api_key:
                return RecognitionResult(
                    success=False, message="未配置 Gemini API Key", engine="gemini"
                )
            ...
            return RecognitionResult(
                success=False, message=f"Gemini 识别失败: {str(e)}", engine="gemini"
            )
            ...
            try:
                ...
                if output_path is None:
                    output_path = os.path.join(
                        tempfile.gettempdir(), f"koto_voice_{int(time.time())}.wav"
                    )
