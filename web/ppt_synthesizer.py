...
class PPTSynthesizer:
    ...
    async def synthesize_from_blueprint(
        ...
        try:
            ...
            for i, slide_blueprint in enumerate(blueprint.slides):
                ...
                progress_pct = 50 + int((current_slide_num / total_slides) * 45)  # 50% -> 95%
                _report(f"正在渲染幻灯片 {current_slide_num}/{total_slides}: {slide_blueprint.title} ({slide_blueprint.slide_type.value})", progress_pct)
            ...
            return {
                ...
                "blueprint_info": blueprint.to_dict() if hasattr(blueprint, 'to_dict') else {}
            }
    ...
    async def _populate_data_slide(self, slide, slide_blueprint, theme_colors):
        ...
        # Big Central Number
        stat_box = slide.shapes.add_textbox(Inches(1), Inches(2.5), Inches(8), Inches(2))
        ...
        # Description
        desc_box = slide.shapes.add_textbox(Inches(2), Inches(4.5), Inches(6), Inches(2))
    ...
    async def _populate_title_slide(
        ...
        if slide_blueprint.content:
            ...
            subtitle_p.text = slide_blueprint.content[0] if isinstance(slide_blueprint.content, list) else str(slide_blueprint.content)
    ...
    async def _populate_content_image_slide(
        ...
        # 右侧图片
        image_list = image_paths.get(slide_blueprint.slide_index, []) if image_paths else []
