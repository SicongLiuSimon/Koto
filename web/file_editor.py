...
class FileEditor:
    ...
    def replace_text(self, file_path: str, old_text: str, new_text: str, use_regex: bool = False, 
                    max_replacements: int = -1) -> Dict[str, Any]:
        """
        替换文件中的文本
        ...
        if use_regex:
            if max_replacements == -1:
                ...
                new_content = re.sub(old_text, new_text, content, count=max_replacements)
            # 计算替换次数
    ...
    def insert_line(self, file_path: str, line_number: int, text: str, mode: str = "after") -> Dict[str, Any]:
        ...
        if line_number < 1 or line_number > len(lines) + 1:
            return {"success": False, "error": f"行号越界: {line_number}（文件共 {len(lines)} 行）"}
    ...
    def smart_edit(self, file_path: str, instruction: str, ai_model=None) -> Dict[str, Any]:
        """
