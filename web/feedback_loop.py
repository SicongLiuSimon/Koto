...
class FeedbackLoopManager:
    ...
    def improve_document_content(
        ...
        if not evaluation_result.get("needs_improvement", False):
            if progress_callback:
                progress_callback(
                    "completed",
                    f"文档质量良好 (评分: {evaluation_result.get('overall_score', 0):.1f}/100)，无需改进",
                )
            return {
        ...
        while self.improvement_iterations < self.max_iterations:
            ...
            prompt = self._build_improvement_prompt(
                current_content, evaluation_result, document_title, iteration_num
            )
            ...
            try:
                ...
                if improved_content:
                    ...
                    improvement_history[-1]["final_score"] = new_score
                    evaluation_result = (
                        new_evaluation.__dict__
                        if hasattr(new_evaluation, "__dict__")
                        else {
    ...
    def improve_ppt_outline(
        ...
        if not evaluation_result.get("needs_improvement", False):
            if progress_callback:
                progress_callback(
                    "completed",
                    f"PPT质量良好 (评分: {evaluation_result.get('overall_score', 0):.1f}/100)，无需改进",
                )
            return {
        ...
        while self.improvement_iterations < self.max_iterations:
            ...
            prompt = self._build_ppt_improvement_prompt(
                current_outline, evaluation_result, title, self.improvement_iterations
            )

            try:
                improved_outline_json = self._call_gemini_for_improvement(
                    prompt, model_id
                )
                ...
                if improved_outline_json:
                    ...
                    if progress_callback:
                        progress_callback(
                            "improving",
                            f"PPT 第 {self.improvement_iterations} 次改进完成",
                        )
                else:
                    break
    ...
    def _build_improvement_prompt(
        self, content: str, evaluation: Dict[str, Any], title: str, iteration: int
    ) -> str:
        """构建改进提示"""
        issues = "\n".join([f"- {issue}" for issue in evaluation.get("issues", [])])
        suggestions = "\n".join(
            [f"- {suggestion}" for suggestion in evaluation.get("suggestions", [])]
        )
    ...
    def _call_gemini_for_improvement(
        ...
        try:
            ...
            response = client.models.generate_content(
                model=model_id,
                contents=prompt,
                config={"temperature": 0.7, "top_p": 0.9, "max_output_tokens": 8000},
            )
