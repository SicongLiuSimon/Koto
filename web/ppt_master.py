...
class PPTContentPlanner:
                ...
                for use_json_mode in [True, False]:
                    try:
                        ...
                        if response and response.text:
                            ...
                            logger.info(
                                f"[PPTContentPlanner] ✅ 初步内容规划成功 (model={model}, {mode_str})"
                            )
                            ...
                            try:
                                ...
                                logger.warning(
                                    f"[PPTContentPlanner] ⚠️ 优化阶段跳过: {critique_err}"
                                )
                        ...
                        mode_str = "JSON模式" if use_json_mode else "文本模式"
                        logger.info(
                            f"[PPTContentPlanner] 模型 {model} ({mode_str}) 失败: {model_err}"
                        )
                        err_text = str(model_err)
                        if (
                            "FAILED_PRECONDITION" in err_text
                            and "location" in err_text.lower()
                        ) or "User location is not supported" in err_text:
                            logger.info(
                                "[PPTContentPlanner] 检测到地区限制，跳过AI规划并回退默认方案"
                            )
                            return self._generate_default_plan(user_request)
                        continue
    ...
    def _run_critique_sync(
        self, initial_plan: Dict[str, Any], user_request: str
    ) -> Dict[str, Any]:
        """
    ...
    async def expand_slide_content(
        self, slide_title: str, points: List[str], context: str = ""
    ) -> List[str]:
        """扩充幻灯片内容（如果内容太少）"""
        if not self.ai_client:
            return points
...
class PPTLayoutPlanner:
    ...
    def plan_layout(
        ...
        layout_config = self.layout_rules.get(
            layout_key, self.layout_rules["balanced"]
        ).copy()

        # 根据内容密度调整
        if slide_blueprint.density == ContentDensity.DENSE:
            layout_config["content_size"] = max(
                18, layout_config.get("content_size", 24) - 4
            )
...
class PPTImageMatcher:
    ...
    async def generate_image_prompts(
        self, slides: List[SlideBlueprint], theme: str = "professional"
    ) -> List[SlideBlueprint]:
        """
        为幻灯片生成图像提示
        """
    ...
    def _generate_image_prompts_for_slide(
        self, slide: SlideBlueprint, theme: str
    ) -> List[str]:
        """生成单个幻灯片的图像提示"""

        if slide.slide_type == SlideType.CONTENT_IMAGE:
            return [f"{slide.title} - 专业插图，{theme}风格，高质量，演示文稿用途"]
        elif slide.slide_type == SlideType.IMAGE_FULL:
            return [f"{slide.title} - 全屏配图，{theme}风格，视觉冲击力强"]
            ...
            return [f"流程图：{slide.title} - 箭头流程，清晰展示步骤"]
...
class PPTMasterOrchestrator:
    ...
    async def orchestrate_ppt_generation(
        ...
        **kwargs,
        ...
        for slide in blueprint.slides:
            if (
                slide.slide_type.value in ["content", "content_image", "comparison"]
                and len(slide.content) < 4
            ):
                # 只针对内容型幻灯片且内容较少时进行扩充
                tasks.append(
                    self.content_planner.expand_slide_content(
                        slide.title, slide.content, context=f"Theme: {blueprint.theme}"
                    )
                )
            else:
        ...
        blueprint.slides = await self.image_matcher.generate_image_prompts(
            blueprint.slides, blueprint.theme
        )
    ...
    def _create_blueprint_from_plan(
        self, user_request: str, content_plan: Dict[str, Any]
    ) -> PPTBlueprint:
        """从内容规划创建PPT蓝图"""
        ...
        for section in outline:
            ...
            for slide_info in section_slides:
                ...
                slide_type = self._map_slide_type(
                    slide_info.get("slide_type", "content")
                )
    ...
    async def _plan_all_layouts(
        self, slides: List[SlideBlueprint]
    ) -> List[SlideBlueprint]:
        """为所有幻灯片规划布局"""
        ...
        for slide in slides:
            ...
            self.layout_planner.plan_layout(
                slide, has_images=has_images, content_count=len(slide.content)
            )

        return slides

    async def _apply_model_callbacks(
        self, blueprint: PPTBlueprint, callbacks: Dict[str, Any]
    ):
        """应用模型回调来增强蓝图"""
    ...
    def _finalize_blueprint(self, blueprint: PPTBlueprint) -> PPTBlueprint:
        ...
        # 应用配色方案
        for slide in blueprint.slides:
            slide.color_scheme = self._choose_color_scheme(
                slide.slide_type, blueprint.theme
            )
    ...
    def get_blueprint_summary(self, blueprint: PPTBlueprint) -> Dict[str, Any]:
        ...
        return {
            ...
            "image_heavy_slides": len(
                [s for s in blueprint.slides if s.slide_type == SlideType.CONTENT_IMAGE]
            ),
