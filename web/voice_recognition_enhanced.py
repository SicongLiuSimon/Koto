...
class EnhancedVoiceRecognizer:
    ...
    def _init_speech_recognition(self):
        ...
        try:
            ...
            logger.error(
                "❌ 未安装 SpeechRecognition，请运行: pip install SpeechRecognition pyaudio"
            )
            return None
    ...
    def recognize_microphone(
        ...
        while retry_count <= self.max_retries:
            try:
                ...
                # 状态：正在识别
                self._update_status(
                    RecognitionStatus.RECOGNIZING, "正在识别语音内容..."
                )
                ...
                try:
                    ...
                    self.stats["avg_duration"] = (
                        self.stats["avg_duration"] * (self.stats["successful"] - 1)
                        + duration_secs
                ...
                if retry_count < self.max_retries:
                    ...
                    self._update_status(
                        RecognitionStatus.RETRYING, f"出错: {last_error}，重试中..."
                    )
