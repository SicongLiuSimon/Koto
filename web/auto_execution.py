...
class AutoExecutionEngine:
    ...
    def execute_task(
        self, user_id: str, task_type: str, params: Dict, force: bool = False
    ) -> Dict:
        """
        执行任务
        ...
        if not force:
            ...
            if not can_exec:
                return {"success": False, "error": f"无法执行: {reason}"}
    ...
    def start_queue_processor(self, interval: int = 60):
        ...
        self.check_thread = threading.Thread(
            target=self._queue_processing_loop, args=(interval,), daemon=True
        )
    ...
    def _handler_archive_old_files(self, params: Dict) -> Dict:
        ...
        archive_dir = os.path.join(
            self.workspace_root, "archive", datetime.now().strftime("%Y%m%d")
        )
    ...
    def _handler_backup_file(self, params: Dict) -> Dict:
        ...
        backup_dir = os.path.join(
            self.workspace_root, "backups", datetime.now().strftime("%Y%m%d")
        )
    ...
    def _handler_remove_duplicates(self, params: Dict) -> Dict:
        """清理重复文件处理器"""
        # 这是一个高风险操作，需要谨慎实现
        return {"message": "重复文件检测功能待实现", "risk_level": "high"}
    ...
    def _handler_create_folder(self, params: Dict) -> Dict:
        ...
        return {"folder_path": folder_path, "full_path": full_path, "created": True}
    ...
    def get_execution_history(self, user_id: str, limit: int = 50) -> List[Dict]:
        """获取执行历史"""
        conn = sqlite3.connect(self.db_path)
        conn.row_factory = sqlite3.Row
        cursor = conn.cursor()
