...
class DocumentEditor:
    ...
    def edit_ppt(file_path: str, modifications: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            ...
            new_file_path = (
                f"{base_name}_edited_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pptx"
            )
            ...
            return {"success": False, "error": f"编辑PPT失败: {str(e)}"}

    @staticmethod
    def edit_word(
        file_path: str, modifications: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
        ...
        try:
                ...
                try:
                    ...
                    if action in (
                        ...
                        if t_idx >= len(doc.tables):
                            errors.append(
                                f"表格索引 {t_idx} 超出范围（共 {len(doc.tables)} 个表格）"
                            )
                            continue
                        ...
                        if action == "update_table_cell":
                            ...
                            if row_i < len(table.rows) and col_i < len(
                                table.rows[row_i].cells
                            ):
                                ...
                                applied_count += 1
                                logger.info(
                                    f"  ✅ 表格{t_idx+1}[{row_i},{col_i}] → '{value}'"
                                )
                            else:
                                errors.append(
                                    f"表格{t_idx+1} 单元格({row_i},{col_i})超出范围"
                                )
            ...
            base_name = os.path.splitext(file_path)[0]
            new_file_path = (
                f"{base_name}_edited_{datetime.now().strftime('%Y%m%d_%H%M%S')}.docx"
            )
            ...
            return {"success": False, "error": f"编辑Word失败: {str(e)}"}
    ...
    def edit_excel(
        ...
        try:
            ...
            base_name = os.path.splitext(file_path)[0]
            new_file_path = (
                f"{base_name}_edited_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
            )
            ...
            return {"success": False, "error": f"编辑Excel失败: {str(e)}"}
        ...
        try:
            ...
            if json_match:
                ...
                json_match = re.search(
                    r'\{.*"modifications".*\}', ai_response, re.DOTALL
                )
                if json_match:
    ...
    @staticmethod
    def edit_word_with_tracked_changes(
        file_path: str, annotations: List[Dict[str, str]]
    ) -> Dict[str, Any]:
        """
            ...
            for anno in annotations:
                ...
                for para in doc.paragraphs:
                    ...
                    new_text = para_text.replace(
                        original, original + "<<<MODIFIED>>>" + modified + "<<<END>>>"
                    )
