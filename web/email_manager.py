...
class EmailManager:
    ...
    def _save_accounts(self):
        """保存邮箱账户配置"""
        try:
            data = {"default_account": self.default_account, "accounts": {}}
    ...
    def _get_email_body(self, msg) -> str:
        ...
        if msg.is_multipart():
            ...
            try:
                ...
                logger.debug("Failed to decode email body: %s", e)

        return body

    def search_emails(
        self, keyword: str, account_email: str = None, folder: str = "INBOX"
    ) -> List[Dict]:
        """
        搜索邮件
        ...
        for email_data in emails:
            if (
                keyword.lower() in email_data["subject"].lower()
                or keyword.lower() in email_data["body"].lower()
                or keyword.lower() in email_data["from"].lower()
            ):
