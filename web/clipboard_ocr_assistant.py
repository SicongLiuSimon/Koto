...
class ClipboardOCRAssistant:
    ...
    def _init_ocr_engine(self):
        ...
        try:
            ...
            try:
                ...
                logger.info(
                    "安装方法: pip install paddleocr 或 pip install pytesseract"
                )
                return None
    ...
    def capture_clipboard_image(self, save_image: bool = True) -> Dict[str, Any]:
        ...
        try:
            ...
            if not isinstance(image, Image.Image):
                return {"success": False, "error": "剪贴板内容不是图片格式"}
        ...
        try:
            ...
            if isinstance(self.ocr_engine, str) and self.ocr_engine == "tesseract":
                ...
                result = self.ocr_engine.ocr(
                    image_path if isinstance(image_path, str) else image, cls=True
                )
    ...
    def capture_and_ocr(
        self, source: str = "screenshot", save_image: bool = True
    ) -> Dict[str, Any]:
        """
        截图/获取剪贴板图片并执行OCR
    ...
    def auto_index_to_knowledge_base(
        self, ocr_result: Dict[str, Any], kb_path: str = "workspace/knowledge_base"
    ):
        """
        将OCR结果自动索引到知识库
        ...
        try:
            ...
            return {"success": True, "doc_path": doc_path, "indexed": True}

        except Exception as e:
            return {"success": False, "error": f"索引到知识库失败: {str(e)}"}

    def monitor_clipboard(
        self, interval: int = 1, duration: int = 60, auto_ocr: bool = True
    ):
        """
        监控剪贴板变化
        ...
        try:
            while time.time() - start_time < duration:
                ...
                if isinstance(current_image, Image.Image):
                    ...
                    if (
                        last_image is None
                        or current_image.tobytes() != last_image.tobytes()
                    ):
                        logger.info("📋 检测到新图片！")
                        ...
                        if auto_ocr and result["success"]:
                            ...
                            if ocr_result["success"]:
                                logger.info(
                                    f"✅ OCR识别成功: {len(ocr_result['text'])} 字符"
                                )
