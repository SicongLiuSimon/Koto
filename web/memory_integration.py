...
class MemoryIntegration:
    ...
    def create_extraction_prompt(
        user_msg: str, ai_msg: str, history: Optional[List] = None
    ) -> str:
        """
...
"""
    ...
    async def extract_and_apply(
        ...
        try:
            ...
            if extracted.get("programming_languages"):
                profile_updates["programming_languages"] = extracted[
            ...
            if extracted.get("communication_style"):
                profile_updates["communication_style"] = extracted[
                    "communication_style"
                ]
            ...
            if profile_updates:
                ...
                logger.info(
                    f"[MemoryIntegration] 🧠 学习到新特征：{list(profile_updates.keys())}"
                )
            ...
            if extracted.get("memories_to_save"):
                ...
                logger.info(
                    f"[MemoryIntegration] 💾 保存了 {len(extracted['memories_to_save'])} 条记忆"
                )

            return {"success": True, "extracted": extracted, "applied": profile_updates}
    ...
    @staticmethod
    def enhance_system_instruction(
        base_instruction: str, memory_context: str, profile_summary: str
    ) -> str:
        """
        增强系统指令，注入记忆和用户画像
        """
