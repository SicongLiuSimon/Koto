...
class SystemInfoCollector:
    ...
    def get_disk_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            info = {
                ...
                "percent_full": round(
                    100 - (free_gb / total_gb * 100) if total_gb > 0 else 0, 1
                ),
            }
            ...
            return {"drives": {}, "total_gb": 0, "free_gb": 0, "error": str(e)}
    ...
    def get_network_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            info["connection_status"] = {
                name: {"is_up": stats.isup, "speed": stats.speed, "mtu": stats.mtu}
            ...
            return {"hostname": socket.gethostname(), "interfaces": {}, "error": str(e)}
    ...
    def get_running_processes(self, top_n: int = 10) -> Dict[str, Any]:
        ...
        try:
            ...
            info = {
                ...
                "key_processes": {
                    "python": [p for p in processes if "python" in p["name"].lower()][
                        :3
                    ],
            ...
            return {"total_processes": 0, "top_processes": [], "error": str(e)}
    ...
    def get_python_environment(self) -> Dict[str, Any]:
        ...
        try:
            ...
            info = {
                ...
                "virtual_env_name": os.environ.get("VIRTUAL_ENV", "").split(os.sep)[-1]
                or None,
            }
            ...
            logger.warning(
                f"[SystemInfo] Warning: Failed to get Python environment info: {e}"
            )
            return {
                "version": platform.python_version(),
                "executable": sys.executable,
    ...
    def get_formatted_info(self, include_top_processes: bool = True) -> str:
        ...
        lines.append(
            f"  • 核心: {cpu['physical_cores']} 物理 / {cpu['logical_cores']} 逻辑"
        )
        ...
        lines.append("\n🧠 **内存状态**:")
        lines.append(
            f"  • 使用: {memory['used_gb']}GB / {memory['total_gb']}GB ({memory['percent']}%)"
        )
        lines.append(f"  • 可用: {memory['available_gb']}GB")
        if memory["swap_total_gb"] > 0:
            lines.append(
                f"  • 虚拟内存: {memory['swap_used_gb']}GB / {memory['swap_total_gb']}GB ({memory['swap_percent']}%)"
            )
        ...
        if disk.get("drives"):
            for drive, info in disk["drives"].items():
                lines.append(
                    f"  • {drive}: {info['used_gb']}GB / {info['total_gb']}GB ({info['percent']}%)"
                )
        ...
        if network.get("interfaces"):
            ipv4_addrs = [
                addr["ipv4"]
                for addr in network["interfaces"].values()
...
def get_formatted_system_info(include_processes: bool = True) -> str:
    """获取格式化的系统信息"""
    return get_system_info_collector().get_formatted_info(
        include_top_processes=include_processes
    )
