...
{"industry": "类别", "category": "子类型(如contract/paper/bp/resume/report/presentation/software)", "entity": "核心实体名(简短)", "confidence": 0.0-1.0}"""
    ...
    def analyze_file(self, file_path: str) -> Dict:
        ...
        ai_used = False
        ai_result = (
            self._ai_classify(file_name, content, file_type) if content else None
        )
        if ai_result:
        ...
        suggested_folder = self._generate_folder_path(
            industry, category, timestamp, keywords, entity_name
        )
    ...
    def _ai_classify(
        self, file_name: str, content: str, file_type: str
    ) -> Optional[Dict]:
        """使用 AI 模型进行智能文件分类
        ...
        try:
            ...
            logger.info(
                f"[FileAnalyzer AI] ✅ {file_name[:30]} → {industry}/{category} ({confidence:.2f}) entity={entity} [{source}]"
            )
            return {
                "industry": industry,
                "category": category,
    ...
    def _extract_content(self, file_path: str) -> str:
        ...
        if file_type in [".txt", ".json", ".csv", ".log"]:
            try:
                ...
                logger.debug("Failed to read text file %s: %s", file_path, e)
                return ""
        ...
        if file_type == ".pdf":
            try:
                ...
                logger.debug("Failed to extract PDF text from %s: %s", file_path, e)
                return ""
        ...
        if file_type in [".docx", ".doc"]:
            try:
                ...
                logger.debug("Failed to extract DOCX text from %s: %s", file_path, e)
                return ""
        ...
        if file_type in [".xlsx", ".xls"]:
            try:
                ...
                logger.debug("Failed to extract Excel text from %s: %s", file_path, e)
                return ""
    ...
    def _save_training_sample(
        self, file_name: str, file_type: str, content: str, ai_result: dict
    ) -> None:
    ...
    def _classify_industry(
        self, keywords: List[str], file_name: str, content: str
    ) -> Tuple[str, float]:
        """根据关键词分类行业"""
        industry_scores = {}
...
if __name__ == "__main__":
    ...
    # 示例文件分析
    test_files = ["sample_contract.txt", "financial_report.txt", "property_lease.txt"]
