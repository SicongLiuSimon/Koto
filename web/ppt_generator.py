...
class PPTGenerator:
    ...
    def generate_from_outline(
        ...
        subtitle_shape.text = self._clean_markdown(
            subtitle or f"{author} | {datetime.now().strftime('%Y-%m-%d')}",
            strip_bold=True,
        )
        ...
        for idx, section in enumerate(outline):
            ...
            if image_gen and (
                s_type in ["image_full", "highlight", "content_image"]
                or (s_type == "detail" and idx % 3 == 0)
            ):
                ...
                local_img_path = os.path.join(
                    os.path.dirname(output_path), "images", img_filename
                )
                ...
                if image_gen.generate_image(
                    img_prompt, local_img_path, aspect_ratio="16:9"
                ):
            ...
            if s_type == "divider":
                ...
                self._create_overview_slide(
                    prs, s_title, section.get("subsections", [])
                )

            elif s_type == "comparison":
                self._create_comparison_slide(
                    prs, s_title, section.get("left", {}), section.get("right", {})
                )

            elif s_type == "highlight":
                self._create_highlight_slide(
                    prs, s_title, s_points, bg_image=generated_img_path
                )
                ...
                textbox = slide.shapes.add_textbox(
                    Inches(1), Inches(5.5), Inches(10), Inches(1.5)
                )
                ...
                if generated_img_path:
                    self._create_picture_slide(
                        prs, s_title, s_points, generated_img_path
                    )
                else:
    ...
    def _create_overview_slide(self, prs, title, subsections):
        ...
        for i, sub in enumerate(subsections):
            ...
            shape = slide.shapes.add_shape(
                MSO_SHAPE.ROUNDED_RECTANGLE, left, top, width - Inches(0.2), height
            )
            shape.fill.solid()
            shape.fill.fore_color.rgb = RGBColor(
                *self.colors.get("light_bg", (240, 240, 240))
            )
            ...
            tf = shape.text_frame
            p = tf.paragraphs[0]
            p.text = self._clean_markdown(
                sub.get("title", f"Section {i+1}"), strip_bold=True
            )
            p.font.bold = True
            p.font.size = Pt(20)
    ...
    def _create_comparison_slide(self, prs, title, left, right):
        ...
        left_box = slide.shapes.add_textbox(
            Inches(1), Inches(2), Inches(5.5), Inches(5)
        )
        tf_l = left_box.text_frame
        tf_l.text = self._clean_markdown(
            left.get("title", "Left Side"), strip_bold=True
        )
        ...
        right_box = slide.shapes.add_textbox(
            Inches(7), Inches(2), Inches(5.5), Inches(5)
        )
        tf_r = right_box.text_frame
        tf_r.text = self._clean_markdown(
            right.get("title", "Right Side"), strip_bold=True
        )
    ...
    def _add_slide_number(self, prs, slide, current, total):
        ...
        box = slide.shapes.add_textbox(
            w - Inches(1.5), prs.slide_height - Inches(0.5), Inches(1.2), Inches(0.35)
        )
        tf = box.text_frame
    ...
    def _add_title_slide(self, prs, title, subtitle, author):
        ...
        # 装饰性 accent 线
        self._add_line(
            slide, Inches(2), Inches(3.8), Inches(6.5), Inches(3.8), "accent", 4
        )

        # 标题
        title_box = slide.shapes.add_textbox(
            Inches(2), Inches(1.5), Inches(9), Inches(2)
        )
        ...
        footer_box = slide.shapes.add_textbox(
            Inches(2), H - Inches(0.75), Inches(8), Inches(0.5)
        )
    ...
    def _add_agenda_slide(self, prs, outline):
        ...
        # 标题 (白色，在色带上)
        title_box = slide.shapes.add_textbox(
            Inches(0.8), Inches(0.25), Inches(8), Inches(0.8)
        )
        ...
        for col in range(2):
            ...
            for i in range(start_idx, end_idx):
                ...
                item_box = slide.shapes.add_textbox(
                    x + Inches(0.7), y + Inches(0.05), Inches(4.5), Inches(0.45)
                )
    ...
    def _add_content_slide(self, prs, section, page_num, total_pages):
        ...
        # 标题下方 accent 分隔线
        self._add_line(
            slide, Inches(0.6), Inches(1.35), Inches(4), Inches(1.35), "accent", 3
        )
        ...
        if has_image:
            try:
                ...
                pic = slide.shapes.add_picture(
                    str(image_path), Inches(7.5), Inches(1.8)
                )
    ...
    def _add_overview_slide(self, prs, section, page_num, total_pages):
        ...
        # 标题
        title_box = slide.shapes.add_textbox(
            Inches(0.6), Inches(0.4), Inches(10), Inches(0.7)
        )
        tp = title_box.text_frame.paragraphs[0]
        ...
        self._add_line(
            slide, Inches(0.6), Inches(1.15), Inches(4), Inches(1.15), "accent", 3
        )
        ...
        for i, sub in enumerate(subsections[: cols * rows]):
            ...
            self._add_rect(
                ...
                Inches(x + margin),
                Inches(y + margin),
                Inches(col_w - 2 * margin),
                Inches(row_h - 2 * margin),
                "light_bg",
            )

            # 子标题
            sub_box = slide.shapes.add_textbox(
                Inches(x + 0.35), Inches(y + 0.25), Inches(col_w - 0.7), Inches(0.45)
            )
            sp = sub_box.text_frame.paragraphs[0]
            sp.text = self._clean_markdown(
                sub.get("subtitle", sub.get("label", "")), strip_bold=True
            )
    ...
    def _add_highlight_slide(self, prs, section, page_num, total_pages):
        ...
        for pt in section.get("points", section.get("content", [])):
            ...
            if "|" in pt:
                parts = pt.split("|", 1)
                highlights.append(
                    {"value": parts[0].strip(), "label": parts[1].strip()}
                )
            elif "：" in pt:
                parts = pt.split("：", 1)
                highlights.append(
        ...
        for i, hl in enumerate(highlights[:n]):
            ...
            # 顶部 accent 色条
            self._add_rect(
                slide, Inches(x), Inches(y_card), Inches(card_w), Inches(0.08), "accent"
            )
            ...
            self._set_font(
                lp.runs[0], label_size, color_key="text" if hl["value"] else "primary"
            )
    ...
    def _add_section_divider_slide(self, prs, section, part_number=1):
        ...
        num_box = slide.shapes.add_textbox(
            Inches(1.2), Inches(1.5), Inches(5), Inches(1.2)
        )
        ...
        # accent 分隔线
        self._add_line(
            slide, Inches(1.2), Inches(3.2), Inches(6), Inches(3.2), "accent", 4
        )

        # 章节标题
        title_box = slide.shapes.add_textbox(
            Inches(1.2), Inches(3.5), Inches(5.5), Inches(2)
        )
        tf = title_box.text_frame
        tf.word_wrap = True
        tp = tf.paragraphs[0]
    ...
    def _add_comparison_slide(self, prs, section, page_num, total_pages):
        ...
        for col_data, x in [(left_data, left_x), (right_data, right_x)]:
            ...
            hp.text = self._clean_markdown(
                col_data.get("label", col_data.get("subtitle", "")), strip_bold=True
            )
            ...
            # 列要点
            pts_box = slide.shapes.add_textbox(
                Inches(x), Inches(y_start + 0.7), Inches(col_w - 0.3), Inches(4.5)
            )
            ptf = pts_box.text_frame
            ptf.word_wrap = True
            ptf.auto_size = None
    ...
    def _add_ending_slide(self, prs, title):
        ...
        # 装饰线
        self._add_line(
            slide, Inches(4), Inches(3.3), Inches(9.3), Inches(3.3), "accent", 4
        )
        ...
        info_box = slide.shapes.add_textbox(
            Inches(1), H - Inches(1.5), Inches(11.3), Inches(0.6)
        )
    ...
    def add_image_to_slide(
        self, prs, slide_index: int, image_path: str, position: str = "right"
    ) -> bool:
        """向指定幻灯片添加图片"""
        try:
...
class EnhancedPPTGenerator:
    ...
    async def generate_with_multimodal(
        ...
        outline_prompt = self._build_outline_prompt(
            title, user_request, search_results, quality_feedback
        )
...
"""
    ...
    def _match_images_to_slides(
        self, outline: List[Dict], images: List[str]
    ) -> List[Dict]:
        """智能匹配图片到幻灯片"""
        # 简单策略：为前N个有内容的章节分配图片
        image_idx = 0
