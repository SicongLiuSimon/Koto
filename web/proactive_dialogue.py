...
class ProactiveDialogueEngine:
        ...
        if condition == "time_of_day":
            ...
            if scene_type == "morning_greeting" and 6 <= hour < 12:
                if self.behavior_monitor:
                    ...
                    context["file_count"] = len(
                        set(e.get("file_path") for e in events if e.get("file_path"))
                    )
            ...
            if self.behavior_monitor:
                ...
                for milestone in milestones:
                    if total_files >= milestone and not self._achievement_sent(
                        user_id, f"files_{milestone}"
                    ):
    ...
    def _save_dialogue(
        self, user_id: str, scene_type: str, message: str, context: Dict
    ):
        """保存对话历史"""
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
