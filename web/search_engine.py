...
class SearchEngine:
                    ...
                    for msg in messages:
                        ...
                        if query_lower in content.lower():
                            results.append({
                                ...
                                'timestamp': timestamp,
                                'match': self._extract_match_context(content, query, 100)
                            })
            except:
                pass
                ...
                with open(index_file, 'r', encoding='utf-8') as f:
                    ...
                    for note_id, note_data in notes_index.items():
                        ...
                        if (query_lower in title.lower() or
                            ...
                            query_lower in ' '.join(tags).lower() or
    ...
    def _extract_match_context(self, text: str, query: str, context_length: int = 100) -> str:
        """
        提取匹配上下文
        ...
        try:
            ...
            if 'chats' in types:
                results['chats'] = self._filter_by_date(
                    self.search_chats('', max_results=1000),
                    start, end
                )
            
            if 'notes' in types:
                results['notes'] = self._filter_by_date(
                    self.search_notes('', max_results=1000),
                    start, end
                )
            
            if 'clipboard' in types:
                results['clipboard'] = self._filter_by_date(
                    self.search_clipboard('', max_results=1000),
                    start, end
                )
    ...
    def _filter_by_date(self, items: List[Dict], start: datetime, end: datetime) -> List[Dict]:
        """按日期过滤结果"""
        filtered = []
