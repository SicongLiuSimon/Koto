...
class TestDocumentFeedbackDeep:
    ...
    def test_analyze_and_suggest_calls_ai_and_returns_suggestions(self):
        ...
        mock_resp.text = json.dumps({
            "modifications": [{"原文片段": "a", "修改后文本": "b"}],
            "summary": "ok"
        })
        mock_client.models.generate_content.return_value = mock_resp
        obj = self._make(client=mock_client)
        obj.reader.read_document.return_value = {
            "success": True, "content": "hello", "formatted_content": "hello",
            "type": "word", "doc_type": "word", "metadata": {}
        }
    ...
    def test_analyze_and_suggest_catches_exception(self):
        ...
        obj = self._make(client=mock_client)
        obj.reader.read_document.return_value = {
            "success": True, "content": "x", "formatted_content": "x",
            "type": "word", "doc_type": "word", "metadata": {}
        }
    ...
    def test_apply_suggestions_calls_editor_for_docx(self):
        ...
        obj.editor.edit_word.return_value = {
            "success": True, "file_path": "/out.docx", "applied_count": 2
        }
        result = obj.apply_suggestions("/fake.docx", [{"原文片段": "a", "修改后文本": "b"}])
        assert result["success"] is True
    ...
    def test_apply_suggestions_calls_editor_for_pptx(self):
        ...
        obj.editor.edit_ppt.return_value = {
            "success": True, "file_path": "/out.pptx", "applied_count": 1
        }
        result = obj.apply_suggestions("/fake.pptx", [{"原文片段": "a", "修改后文本": "b"}])
        assert result["success"] is True

    # -- full_feedback_loop -------------------------------------------------
    def test_full_feedback_loop_analysis_failure(self):
        obj = self._make(client=MagicMock())
        with patch.object(obj, "analyze_and_suggest", return_value={"success": False, "error": "e"}):
            result = obj.full_feedback_loop("/fake.docx")
        assert result["success"] is False

    def test_full_feedback_loop_auto_apply_true(self):
        obj = self._make(client=MagicMock())
        analysis = {"success": True, "modifications": [{"原文片段": "a"}], "summary": "ok summary text",
                     "original_content": "c", "ai_suggestions": "s", "modification_count": 1}
        edit_result = {"success": True, "file_path": "/o.docx", "applied_count": 1}
        with patch.object(obj, "analyze_and_suggest", return_value=analysis), \
    ...
    def test_annotate_document_delegates_to_annotator(self):
        ...
        obj.annotator.annotate_document.return_value = {
            "success": True, "original_file": "/a", "revised_file": "/b",
            "applied": 2, "failed": 0
        }
        result = obj.annotate_document("/fake.docx", [{"原文片段": "x", "修改建议": "y"}])
        assert result["success"] is True
    ...
    def test_split_chunks_long_content(self):
        from web.document_feedback import DocumentFeedbackSystem
        text = "\n\n".join([f"Paragraph number {i} with some text." for i in range(100)])
...
class TestPptGeneratorDeep:
    ...
    def test_generate_from_text_creates_pptx(self):
        ...
        with patch("web.ppt_generator.Presentation", return_value=mock_prs):
            with tempfile.TemporaryDirectory() as td:
                ...
                result = gen.generate_from_text("Title\n\nSlide 1 content\n\nSlide 2 content", out)
    ...
    def test_generate_from_outline_with_image_full_type(self):
        gen = self._make_gen()
        mock_prs = MagicMock()
        outline = [{"title": "Image Slide", "points": ["Beautiful scenery"],
                     "slide_type": "image_full"}]
        with patch("web.ppt_generator.Presentation", return_value=mock_prs):
            with tempfile.TemporaryDirectory() as td:
                out = os.path.join(td, "out.pptx")
                result = gen.generate_from_outline("Title", outline, out, enable_ai_images=False)
        assert "output_path" in result

    def test_generate_from_outline_with_content_image_type(self):
        gen = self._make_gen()
        mock_prs = MagicMock()
        outline = [{"title": "Content Image", "points": ["Data visualization"],
...
class TestTrackChangesEditorDeep:
    ...
    def test_apply_hybrid_changes_classifies_suggestion_as_comment(self):
        ...
        with patch("web.track_changes_editor.Document", return_value=mock_doc), \
             patch.object(editor, "_apply_single_comment", return_value=True), \
             patch.object(editor, "_inject_comments_to_docx", return_value=True):
    ...
    def test_apply_hybrid_changes_exception(self):
        ...
        with patch("web.track_changes_editor.Document", side_effect=Exception("fail")):
            result = editor.apply_hybrid_changes("/fake.docx", [{"原文片段": "a", "修改后文本": "b"}])
        assert result["success"] is False
    ...
    def test_make_run_with_rpr(self):
        editor = self._make_editor()
        run = editor._make_run("test text", '<w:rPr xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"><w:b/></w:rPr>')
...
class TestVoiceFastDeep:
    ...
    def test_voice_result_to_dict(self):
        ...
        vr = VoiceResult(success=True, text="hello", engine="vosk", message="ok", confidence=0.9)
        d = vr.to_dict()
    ...
    def test_get_fast_status_returns_dict(self):
        with patch("web.voice_fast._recognizer_instance", None), \
             patch("web.voice_fast.FastVoiceRecognizer") as MockRec:
            mock_inst = MagicMock()
    ...
    def test_recognize_voice_delegates(self):
        with patch("web.voice_fast.get_recognizer") as mock_get:
            ...
            mock_rec.recognize.return_value = MagicMock(
                success=True, text="hi", engine="vosk", message="", confidence=0.9,
    ...
    def test_get_available_engines_returns_dict(self):
        with patch("web.voice_fast.get_recognizer") as mock_get:
            mock_rec = MagicMock()
            mock_rec.get_available_engines.return_value = {"engines": [], "primary": None}
            mock_get.return_value = mock_rec
    ...
    def test_detect_engines_no_engines_available(self):
        with patch("web.voice_fast.FastVoiceRecognizer._check_vosk", return_value=False), \
             patch("web.voice_fast.FastVoiceRecognizer._check_win32_sapi", return_value=False), \
             patch("web.voice_fast.FastVoiceRecognizer._check_windows_sapi", return_value=False), \
             patch("web.voice_fast.FastVoiceRecognizer._check_speech_recognition", return_value=False), \
    ...
    def test_detect_engines_vosk_available(self):
        with patch("web.voice_fast.FastVoiceRecognizer._check_vosk", return_value=True), \
             patch("web.voice_fast.FastVoiceRecognizer._check_win32_sapi", return_value=False), \
             patch("web.voice_fast.FastVoiceRecognizer._check_windows_sapi", return_value=False), \
             patch("web.voice_fast.FastVoiceRecognizer._check_speech_recognition", return_value=False), \
             patch("web.voice_fast.FastVoiceRecognizer._start_background_init"):
    ...
    def test_detect_engines_speech_recognition_available(self):
        with patch("web.voice_fast.FastVoiceRecognizer._check_vosk", return_value=False), \
             patch("web.voice_fast.FastVoiceRecognizer._check_win32_sapi", return_value=False), \
             patch("web.voice_fast.FastVoiceRecognizer._check_windows_sapi", return_value=False), \
             patch("web.voice_fast.FastVoiceRecognizer._check_speech_recognition", return_value=True), \
             patch("web.voice_fast.FastVoiceRecognizer._start_background_init"):
            from web.voice_fast import FastVoiceRecognizer
            rec = FastVoiceRecognizer()
...
class TestLocalModelInstallerDeep:
    ...
    def test_is_ollama_running_false(self):
        with patch("src.local_model_installer.socket.create_connection", side_effect=ConnectionRefusedError):
            from src.local_model_installer import is_ollama_running
            assert is_ollama_running() is False

    def test_is_ollama_running_timeout(self):
        with patch("src.local_model_installer.socket.create_connection", side_effect=TimeoutError):
            from src.local_model_installer import is_ollama_running
            assert is_ollama_running() is False

    # -- _find_ollama_exe ---------------------------------------------------
    def test_find_ollama_exe_via_shutil_which(self):
        with patch("src.local_model_installer.shutil.which", return_value="C:\\ollama\\ollama.exe"):
    ...
    def test_find_ollama_exe_not_found(self):
        with patch("src.local_model_installer.shutil.which", return_value=None), \
             patch("src.local_model_installer._OLLAMA_SEARCH_PATHS", []):
            from src.local_model_installer import _find_ollama_exe
            result = _find_ollama_exe()
    ...
    def test_find_ollama_exe_from_search_path(self):
        ...
        with patch("src.local_model_installer.shutil.which", return_value=None), \
             patch("src.local_model_installer._OLLAMA_SEARCH_PATHS", [mock_path]):
            from src.local_model_installer import _find_ollama_exe
            result = _find_ollama_exe()
        assert result is not None
    ...
    def test_start_ollama_no_exe_found(self):
        with patch("src.local_model_installer.is_ollama_running", return_value=False), \
    ...
    def test_save_result_exception_silent(self):
        from src.local_model_installer import save_result
        with patch("src.local_model_installer.RESULT_FILE", MagicMock(write_text=MagicMock(side_effect=PermissionError))):
    ...
    def test_pull_model_success(self):
        ...
        with patch("src.local_model_installer._find_ollama_exe", return_value="ollama"), \
    ...
    def test_get_system_info_returns_required_keys(self):
        ...
        with patch("src.local_model_installer.platform.processor", return_value="Intel Core i7"), \
...
class TestKotoSetupDeep:
    ...
    def test_write_config_creates_file(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            with patch("src.koto_setup.APP_ROOT", td_path):
                _write_gemini_config("AIzaTestKey123456789012345678", "https://custom.api.com")
    ...
    def test_write_config_default_base(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            content = (td_path / "config" / "gemini_config.env").read_text(encoding="utf-8")
    ...
    def test_read_config_values_parses_both_fields(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            cfg = td_path / "config" / "gemini_config.env"
            cfg.parent.mkdir(parents=True)
            cfg.write_text("GEMINI_API_KEY=AIzaKey123\nGEMINI_API_BASE=https://api.example.com\n")
            with patch("src.koto_setup.APP_ROOT", td_path):
    ...
    def test_run_setup_if_needed_forced_with_flag(self):
        ...
        with patch("sys.argv", ["koto_setup.py", "--setup"]), \
             patch("src.koto_setup._show_api_setup_wizard", return_value={"key": "AIzaKey12345678901234567890", "base": ""}) as mock_wizard, \
             patch("src.koto_setup._write_gemini_config"), \
             patch("src.koto_setup.APP_ROOT", Path(tempfile.mkdtemp())):
            try:
                _run_setup_if_needed()
            except Exception:
                pass
    ...
    def test_run_setup_if_needed_user_cancels(self):
        from src.koto_setup import _run_setup_if_needed
        with patch("sys.argv", ["koto_setup.py", "--setup"]), \
             patch("src.koto_setup._show_api_setup_wizard", return_value={"key": None, "base": "", "cancelled": True}):
            _run_setup_if_needed()  # should not crash

    def test_run_setup_skips_when_key_valid(self):
        from src.koto_setup import _run_setup_if_needed
        with patch("sys.argv", ["koto_setup.py"]), \
             patch("src.koto_setup._api_key_configured", return_value=True), \
             patch("src.koto_setup._read_config_values", return_value=("AIzaKey", "")), \
             patch("src.koto_setup._validate_api_key", return_value=(True, "")), \
             patch("src.koto_setup._show_api_setup_wizard") as mock_wizard:
            _run_setup_if_needed()
    ...
    def test_run_setup_skips_on_network_error(self):
        ...
        with patch("sys.argv", ["koto_setup.py"]), \
             patch("src.koto_setup._api_key_configured", return_value=True), \
             patch("src.koto_setup._read_config_values", return_value=("AIzaKey", "")), \
             patch("src.koto_setup._validate_api_key", return_value=(False, "⚠️ 网络异常")), \
             patch("src.koto_setup._show_api_setup_wizard") as mock_wizard:
            _run_setup_if_needed()
        mock_wizard.assert_not_called()
