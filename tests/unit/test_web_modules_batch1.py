...
class TestSettingsManager:
    ...
    def test_reset_all(self):
        with patch("web.settings.os.path.exists", return_value=False), patch(
            ...
            mgr.reset()
            assert (
                mgr.get("ai", "default_model")
                == DEFAULT_SETTINGS["ai"]["default_model"]
            )
...
class TestSystemInfoCollector:
    ...
    def test_warnings_high_cpu(self, collector):
        ...
        collector._set_cached(
            "disk_info",
            {"drives": {}, "total_gb": 500, "free_gb": 400, "percent_full": 20},
        )
...
class TestWorkFileLibrary:
    ...
    def library(self, tmp_path):
        ...
        with patch.object(
            WorkFileLibrary, "_resolve_db_path", return_value=str(tmp_path / "test.db")
        ):
            lib = WorkFileLibrary()
        return lib
    ...
    def test_batch_upsert_replaces_on_duplicate(self, library):
        ...
        library._batch_upsert(rows)
        rows2 = [
            ("/tmp/f.docx", "f.docx", "f.docx", ".docx", "Word文档", 999, now, now)
        ]
        library._batch_upsert(rows2)
        assert library.count() == 1
        with library._conn() as conn:
            row = conn.execute(
                "SELECT size FROM work_files WHERE path = '/tmp/f.docx'"
            ).fetchone()
        assert row["size"] == 999
