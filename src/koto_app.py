...
if getattr(sys, 'frozen', False):
    ...
    # 开发环境：koto_app.py 位于 src/ 子目录，APP_ROOT 指向项目根目录
            ...
            if 0 <= error_line < len(lines):
                ...
                if '\\\\n' in original_line or "\\\\n" in original_line:
                    ...
                    if new_line != original_line:
                        ...
                        fixed = True
                        _write_log(f"  修复第 {error_line + 1} 行: 替换 f-string 中的反斜杠")
    ...
    try:
        ...
        if sock.connect_ex((KOTO_HOST, KOTO_PORT)) == 0:
            ...
            if _check_http_ok(health_url):
                _write_log(f"ℹ️ 检测到 {KOTO_HOST}:{KOTO_PORT} 已在运行，健康检查通过，跳过内置服务启动")
                return {"started": False, "already_running": True}
            else:
                _write_log(f"⚠️ {KOTO_HOST}:{KOTO_PORT} 被占用但健康检查失败，尝试清理占用进程")
                ...
                if cleaned:
                    ...
                    if alt_port is None:
                        _write_log(f"⚠️ 清理失败，且未找到可用备用端口（起始 {FALLBACK_PORT}）")
...
def main():
    ...
    if server_info.get("needs_fallback"):
        ...
        if not backend_ready and server_info.get("thread") is not None and server_info["thread"].is_alive():
        ...
        if not backend_ready:
            ...
            fallback_port = _find_available_port(KOTO_HOST, FALLBACK_PORT) or FALLBACK_PORT
            threading.Thread(
                target=_start_fallback_server,
                args=(err_msg, fallback_port),
                daemon=True
