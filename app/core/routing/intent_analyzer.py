...
class IntentAnalyzer:
    ...
    TRIGGER_PATTERNS = [
        ...
        r'刚才', r'那个', r'上一个', r'上个', r'继续', r'修改', r'换成', r'改成', r'再写一个', r'再画一个', r'再生成一个',
        r'这个计划', r'该计划', r'上述计划', r'上面的计划', r'这个方案', r'该方案', r'上述方案',
        r'这个大纲', r'该大纲', r'这个ppt', r'该ppt', r'这个PPT', r'该PPT', r'按照这个', r'根据这个'
    ]
    ...
    def rewrite_intent(cls, user_input: str, history: list, memory_context: str = "") -> str:
        ...
        if LocalModelRouter.is_ollama_available():
            ...
            if LocalModelRouter._initialized and LocalModelRouter._model_name:
                print(f"[IntentAnalyzer] 🧠 正在使用本地模型 ({LocalModelRouter._model_name}) 分析意图...")
                start_time = time.time()
                result, err = LocalModelRouter.call_ollama_chat(
                    messages=[
                        {"role": "system", "content": "你是一个意图重写助手。只输出重写后的文本，不要解释。"},
                ...
                if err:
                    ...
                    if result and len(result) > 2:
                        print(f"[IntentAnalyzer] ✅ 意图重写成功 ({time.time() - start_time:.2f}s): '{user_input}' -> '{result}'")
                        return result
                    else:
            ...
            for msg in reversed(history):
                if msg.get("role") == "user":
                    ...
                    if not any(re.search(p, content, re.IGNORECASE) for p in repeat_patterns):
                        ...
                        break
            if last_user_msg:
                print(f"[IntentAnalyzer] 🔄 基础正则匹配成功: '{user_input}' -> '{last_user_msg}'")
